...
| `messages` | Yes | array<[PromptCachingBetaInputMessage](./PromptCachingBetaInputMessage.md)> | Input messages.
...
| `max_tokens` | Yes | integer | The maximum number of tokens to generate before stopping.
...
| `stop_sequences` | No | array<string> | Custom text sequences that will cause the model to stop generating.
...
| `system` | No | anyOf(2) | System prompt.
...
| `tool_choice` | No | [ToolChoice](ToolChoice.md) |  |
| `tools` | No | array<[PromptCachingBetaTool](./PromptCachingBetaTool.md)> | Definitions of tools that the model may use.
