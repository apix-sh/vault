...
| `finished` | No | string | A timestamp recording when the task was finished. |
| `id` | Yes | string | The ID of the task. |
| `lastUpdate` | Yes | string | A timestamp recording when the task progress was last updated. |
...
| `started` | No | string | A timestamp recording when the task was started. |
| `status` | Yes | string | The status of the task. Allowed values: ENQUEUED, RUNNING, COMPLETE, FAILED, CANCEL_REQUESTED, CANCELLED, DEAD |
| `submitted` | No | string | A timestamp recording when the task was submitted. |
