...
| `act` | No | any | Allowed values: abuse_children |
| `country` | No | string | Text not exceeding 255 characters. This field may be released by Cloudflare to third parties such as the Lumen Database (https://lumendatabase.org/).<br/> |
...
| `ncmec_notification` | Yes | string | Notification type based on the abuse type. NOTE: Copyright (DMCA) and Trademark reports cannot be anonymous.<br/> Allowed values: send, send-anon |
| `owner_notification` | Yes | string | Notification type based on the abuse type. NOTE: Copyright (DMCA) and Trademark reports cannot be anonymous.<br/> Allowed values: send, send-anon, none |