...
| :------- | :------: | :--- | :---------- |
| `access_key` | No | string | Access key of the storage medium. Access key is not required for the `gcs` storage media type.<br/><br/>Note that this field is not readable by clients, only writeable. |
| `auth_method` | No | string | Authentication method used for "sftp" type storage medium<br/> Allowed values: KEY, PASSWORD |
