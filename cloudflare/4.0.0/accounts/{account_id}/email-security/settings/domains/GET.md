...
| `allowed_delivery_mode` | No | allOf(1) | Filters response to domains with the provided delivery mode. |
| `domain` | No | array<string> | Filters results by the provided domains, allowing for multiple occurrences. |
| `active_delivery_mode` | No | allOf(1) | Filters response to domains with the currently active delivery mode. |
| `integration_id` | No | string | Filters response to domains with the provided integration ID. |
