...
| `_worker.bundle` | No | string | Worker bundle file in multipart/form-data format. Mutually exclusive with `_worker.js`.<br/>Cannot specify both `_worker.js` and `_worker.bundle` in the same request.<br/>Maximum size: 25 MiB.<br/> |
| `_worker.js` | No | string | Worker JavaScript file. Mutually exclusive with `_worker.bundle`.<br/>Cannot specify both `_worker.js` and `_worker.bundle` in the same request.<br/> |
...
| `manifest` | No | string | JSON string containing a manifest of files to deploy. Maps file paths to their content hashes.<br/>Required for direct upload deployments. Maximum 20,000 entries.<br/> |
