...
| `interpolation` | No | boolean | Enables interpolation for all series (using the average). |
| `metric` | Yes | string | Defines which metric to return (bandwidth, latency, or DNS response time). |
