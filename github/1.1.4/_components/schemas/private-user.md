---
type: "object"
---

# private-user


Private User

## Properties

| Property | Required | Type | Description |
| :------- | :------: | :--- | :---------- |
| `login` | Yes | string |  |
| `id` | Yes | integer |  |
| `user_view_type` | No | string |  |
| `node_id` | Yes | string |  |
| `avatar_url` | Yes | string |  |
| `gravatar_id` | Yes | string |  |
| `url` | Yes | string |  |
| `html_url` | Yes | string |  |
| `followers_url` | Yes | string |  |
| `following_url` | Yes | string |  |
| `gists_url` | Yes | string |  |
| `starred_url` | Yes | string |  |
| `subscriptions_url` | Yes | string |  |
| `organizations_url` | Yes | string |  |
| `repos_url` | Yes | string |  |
| `events_url` | Yes | string |  |
| `received_events_url` | Yes | string |  |
| `type` | Yes | string |  |
| `site_admin` | Yes | boolean |  |
| `name` | Yes | string |  |
| `company` | Yes | string |  |
| `blog` | Yes | string |  |
| `location` | Yes | string |  |
| `email` | Yes | string |  |
| `notification_email` | No | string |  |
| `hireable` | Yes | boolean |  |
| `bio` | Yes | string |  |
| `twitter_username` | No | string |  |
| `public_repos` | Yes | integer |  |
| `public_gists` | Yes | integer |  |
| `followers` | Yes | integer |  |
| `following` | Yes | integer |  |
| `created_at` | Yes | string |  |
| `updated_at` | Yes | string |  |
| `private_gists` | Yes | integer |  |
| `total_private_repos` | Yes | integer |  |
| `owned_private_repos` | Yes | integer |  |
| `disk_usage` | Yes | integer |  |
| `collaborators` | Yes | integer |  |
| `two_factor_authentication` | Yes | boolean |  |
| `plan` | No | object |  |
| `business_plus` | No | boolean |  |
| `ldap_dn` | No | string |  |