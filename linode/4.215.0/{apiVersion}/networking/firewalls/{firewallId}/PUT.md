...
| `status` | No | string | The status to be applied to this Firewall.<br/><br/> - When a Firewall is first created its status is `enabled`.<br/> - An assigned default firewall can't be disabled.<br/> - Run the [Delete a firewall](https://techdocs.akamai.com/linode-api/reference/delete-firewall) operation to delete a Firewall. |
