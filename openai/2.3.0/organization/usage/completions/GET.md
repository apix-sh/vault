...
| `batch` | No | boolean | If `true`, return batch jobs only. If `false`, return non-batch jobs only. By default, return both.
<br/>*Serialization: style=Form* |
| `group_by` | No | array<string> | Group the usage data by the specified fields. Support fields include `project_id`, `user_id`, `api_key_id`, `model`, `batch` or any combination of them.<br/>*Serialization: style=Form* |
