---
method: "GET"
url: "https://api.openai.com/v1/vector_stores/{vector_store_id}/file_batches/{batch_id}/files"
auth: "none"
...
| :--- | :------: | :--- | :---------- |
| `vector_store_id` | Yes | string | The ID of the vector store that the files belong to. |
| `batch_id` | Yes | string | The ID of the file batch that the files belong to. |
...
| `order` | No | string | Sort order by the `created_at` timestamp of the objects. `asc` for ascending order and `desc` for descending order.<br/> |
...
| `filter` | No | string | Filter by file status. One of `in_progress`, `completed`, `failed`, `cancelled`. |
