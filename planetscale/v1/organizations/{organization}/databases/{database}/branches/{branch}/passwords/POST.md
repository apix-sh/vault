...
| :--- | :---: | :--- | :--- |
| `name` | No | string | Optional name of the password |
| `role` | No | string | The database role of the password (i.e. admin) |
| `replica` | No | boolean | Whether the password is for a read replica |
| `ttl` | No | integer | Time to live (in seconds) for the password. The password will be invalid when TTL has passed |
| `cidrs` | No | array<string> | List of IP addresses or CIDR ranges that can use this password |
| `direct_vtgate` | No | boolean | Whether the password connects directly to a VTGate |
