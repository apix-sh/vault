...
| :--- | :---: | :--- | :--- |
| `branch` | Yes | string | The name of the branch the deploy request is created from |
| `into_branch` | Yes | string | The name of the branch the deploy request will be merged into |
| `notes` | No | string | Notes about the deploy request |
| `auto_cutover` | No | boolean | Whether or not to enable auto_cutover for the deploy request. When enabled, will auto cutover to the new schema as soon as it is ready. |
| `auto_delete_branch` | No | boolean | Whether or not to enable auto_delete_branch for the deploy request. When enabled, will delete the branch once the DR successfully completes. |
