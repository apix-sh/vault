...
| `attachmentsRole` | No | string | The role required to download event attachments, such as native crash reports or log files.<br/><br/>* `member` - Member<br/>* `admin` - Admin<br/>* `manager` - Manager<br/>* `owner` - Owner |
| `avatar` | No | string | The image to upload as the organization avatar, in base64. Required if `avatarType` is `upload`. |
| `avatarType` | No | string | The type of display picture for the organization.<br/><br/>* `letter_avatar` - Use initials<br/>* `upload` - Upload an image |
...
| `debugFilesRole` | No | string | The role required to download debug information files, ProGuard mappings and source maps.<br/><br/>* `member` - Member<br/>* `admin` - Admin<br/>* `manager` - Manager<br/>* `owner` - Owner |
| `defaultRole` | No | string | The default role new members will receive.<br/><br/>* `member` - Member<br/>* `admin` - Admin<br/>* `manager` - Manager<br/>* `owner` - Owner |
