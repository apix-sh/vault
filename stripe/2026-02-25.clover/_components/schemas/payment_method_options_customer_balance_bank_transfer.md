...
| `requested_address_types` | No | array<string> | List of address types that should be returned in the financial_addresses response. If not specified, all valid types will be returned.<br/><br/>Permitted values include: `sort_code`, `zengin`, `iban`, or `spei`. |
