---
method: "POST"
url: "https://api.twilio.com/2010-04-01/Accounts/{AccountSid}/Calls/{CallSid}/Streams.json"
...
| `StatusCallback` | No | string | Absolute URL to which Twilio sends status callback HTTP requests. |
| `StatusCallbackMethod` | No | string | The HTTP method Twilio uses when sending `status_callback` requests. Possible values are `GET` and `POST`. Default is `POST`. |
| `Track` | No | [stream_enum_track](../../../../../../_components/schemas/stream_enum_track.md) |  |
| `Url` | Yes | string | Relative or absolute URL where WebSocket connection will be established. |
