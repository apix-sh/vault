...
| `api_version` | No | string | The API version used to make the recording. |
| `call_sid` | No | string | The SID of the [Call](https://www.twilio.com/docs/voice/api/call-resource) the Recording resource is associated with. |
...
| `price` | No | number | The one-time cost of creating the recording in the `price_unit` currency. |
...
| `source` | No | [call_recording_enum_source](call_recording_enum_source.md) |  |
...
| `status` | No | [call_recording_enum_status](call_recording_enum_status.md) |  |
| `track` | No | string | The recorded track. Can be: `inbound`, `outbound`, or `both`. |
