...
| :------- | :------: | :--- | :---------- |
| `account_sid` | No | string | The SID of the [Account](https://www.twilio.com/docs/iam/api/account) that created this IncomingPhoneNumber resource. |
| `address_requirements` | No | [incoming_phone_number_enum_address_requirement](incoming_phone_number_enum_address_requirement.md) |  |
| `address_sid` | No | string | The SID of the Address resource associated with the phone number. |
...
| `bundle_sid` | No | string | The SID of the Bundle resource that you associate with the phone number. Some regions require a Bundle to meet local Regulations. |
...
| `emergency_address_sid` | No | string | The SID of the emergency address configuration that we use for emergency calling from this phone number. |
| `emergency_address_status` | No | [incoming_phone_number_enum_emergency_address_status](incoming_phone_number_enum_emergency_address_status.md) |  |
| `emergency_status` | No | [incoming_phone_number_enum_emergency_status](incoming_phone_number_enum_emergency_status.md) |  |
| `friendly_name` | No | string | The string that you assigned to describe the resource. |
| `identity_sid` | No | string | The SID of the Identity resource that we associate with the phone number. Some regions require an Identity to meet local regulations. |
| `origin` | No | string | The phone number's origin. `twilio` identifies Twilio-owned phone numbers and `hosted` identifies hosted phone numbers. |
...
| `sid` | No | string | The unique string that that we created to identify this IncomingPhoneNumber resource. |
...
| `type` | No | string | The phone number type. |
...
| `voice_caller_id_lookup` | No | boolean | Whether we look up the caller's caller-ID name from the CNAM database ($0.01 per look up). Can be: `true` or `false`. |
...
| `voice_receive_mode` | No | [incoming_phone_number_enum_voice_receive_mode](incoming_phone_number_enum_voice_receive_mode.md) |  |
