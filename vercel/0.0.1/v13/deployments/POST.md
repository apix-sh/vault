...
| :--- | :------: | :--- | :---------- |
| `forceNew` | No | any | Forces a new deployment even if there is a previous similar deployment |
| `skipAutoDetectionConfirmation` | No | any | Allows to skip framework detection so the API would not fail to ask for confirmation |
